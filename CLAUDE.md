...
- `Machine.deploy()` shells out to `virt-install`. The `--os-variant` value is derived by splitting `self.os` on `-` and taking the first segment — this is why custom images must be named `{os_variant}-{anything}` (see `docs/walkthrough.md` "Image Naming"). The `--network` argument is built by `_virt_install_network_arg(iface)` near the top of `libvirt.py`: `network_type=network` (default) emits the managed-network form with a fixed PCI address; `network_type=user` / `passt` emit virt-install's user-mode forms (no libvirt network required) — required for `qemu:///session` where rootless libvirt cannot manage a NAT network. The same constants live in `utils/network.py` (`NETWORK_TYPES`, `USER_MODE_NETWORK_TYPES`); these drive only the manifest path's `interfaces.network_type` validation now — the standalone `createvm` dropped `--network-type` in the lvscripts-parity port and is `qemu:///system`/managed-network only.
...
- `network-config.v1.j2` and `network-config.v2.j2` — selected by `image.network_version` (1 = ENI-style, 2 = netplan-style). Each image entry in the manifest pins which version cloud-init should emit. The v2 template selects each NIC with netplan `match.driver: virtio_net` (every lvlab NIC is `model=virtio`) and configures it under whatever name the distro assigns. It deliberately does **not** `set-name`/rename the interface: renaming works under NetworkManager (Fedora) but leaves the NIC unconfigured under systemd-networkd (Debian/Ubuntu), so the guest never completes DHCP — the manifest's `iface.name` is therefore only the netplan stanza key (a label), not the in-guest device name (`enp1s0` / `ens3` / `eth0`). This was latent on Debian until the createvm/deletevm connectivity integration tests caught it — no earlier test verified in-guest networking. Multi-NIC manifests are a documented limitation — `match.driver: virtio_net` matches every virtio NIC, so it can't disambiguate >1; multi-NIC needs per-interface MAC matching, not yet supported.
