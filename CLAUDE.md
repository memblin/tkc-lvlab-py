...
- `network-config.v1.j2` and `network-config.v2.j2` — selected by `image.network_version` (1 = ENI-style, 2 = netplan-style). Each image entry in the manifest pins which version cloud-init should emit. The v2 template selects each NIC by `match.macaddress` — the per-interface MAC pinned by `Machine.__init__`/createvm and passed verbatim to `virt-install --network ...,mac=` — falling back to `match.driver: virtio_net` only for a (now unexpected) MAC-less interface. MAC matching is the **only NIC selector cloud-init honours across both of its renderers**: cloud-init writes a verbatim netplan file on Debian/Ubuntu (systemd-networkd, which honours `match: driver`) but a NetworkManager keyfile on Fedora/RHEL, and the NM renderer **ignores `match:`** — it reduces the netplan stanza key to a literal `interface-name`, so a driver match silently binds only when the guest NIC happens to be named like that label. That worked on AlmaLinux (NIC is `eth0`, matching the label) but failed on Fedora (NIC is `enp1s0`): the static profile bound nothing and NM's default DHCP connection won, so the VM came up on the wrong (DHCP) address — and Fedora's earlier DHCP "pass" was a false positive from that same fallback. A MAC-bound keyfile activates regardless of device name, fixing both. The MAC value is **quoted** in the template because an all-numeric MAC (every octet ≤ `0x3b`) would otherwise be misparsed as a YAML 1.1 base-60 integer by cloud-init's loader. The template still deliberately does **not** `set-name`/rename: renaming leaves the NIC unconfigured under systemd-networkd, so `iface.name` is only the netplan stanza key (a label), never the in-guest device name (`enp1s0` / `ens3` / `eth0`). This whole class of bug was latent until the createvm/deletevm connectivity integration tests exercised in-guest networking per distro. Per-interface MAC matching also disambiguates multiple NICs (the old driver match could not), though multi-NIC manifests are still not exercised end-to-end.
