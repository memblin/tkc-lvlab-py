...
## Phase 11 — Migrate docs from MkDocs + Material to Zensical

**Status:** open. Surfaced 2026-05-23 during the Phase 8 `uv run mkdocs build --strict` run, which now prints a deprecation banner from the Material for MkDocs team:
...
**Direction:** move to [Zensical](https://zensical.org/) (the Material maintainer's MkDocs successor) before MkDocs 2.0 lands and breaks our build. Better to migrate intentionally on our schedule than to have a stale dep block a future release.

### Work this implies

- [ ] Spike a Zensical config for this repo's docs surface in a throwaway branch — confirm the mkdocstrings replacement (or equivalent API-reference generator) handles Google-style docstrings + type hints the way we use them today.
- [ ] Identify Zensical's equivalents for what we currently use in `mkdocs.yml`: `mkdocstrings` (API reference), `pymdownx.*` extensions, `navigation.*` Material features, the colour palette + toggle.
- [ ] Decide whether `docs/api/*.md` survives or gets regenerated by the new tooling. If the directive syntax changes from `:::tkc_lvlab.utils.X`, every API page needs editing.
- [ ] `pyproject.toml` `[dependency-groups] dev` swap: drop `mkdocs`, `mkdocs-material`, `mkdocstrings[python]`; add Zensical + whichever API-reference plugin we land on.
- [ ] `.pre-commit-config.yaml` mdformat exclude already covers `docs/api/` for `:::` directives — re-evaluate if Zensical's directive syntax differs.
- [ ] Replace `uv run mkdocs build --strict` and `uv run mkdocs serve` in `CLAUDE.md` and `docs/CONTRIBUTING.md` with whichever Zensical commands replace them.
- [ ] Update `mkdocs.yml` → whatever Zensical's config file is named.
- [ ] Confirm GitHub Pages deployment still works (current workflow path TBD — there is no docs-deploy workflow today, but the site is referenced as `https://memblin.github.io/tkc-lvlab-py/`).

### Risk flags

- **Timing-sensitive.** Doing this before MkDocs 2.0 lands keeps our build green; doing it after means a forced migration under time pressure. The deprecation banner is the early-warning shot.
- **mkdocstrings replacement is the biggest unknown.** Our Phase 7 docstring conversion only delivers value if the API-reference rendering survives the migration. Confirm equivalent functionality in the spike before committing to the swap.
- **Theme aesthetic drift.** Material has a recognisable look; Zensical may differ. Capture before/after screenshots in the PR so the visual change is visible.
