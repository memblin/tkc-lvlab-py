...
`tkc-lvlab` (binary: `lvlab`) is a Typer-based CLI (Typer wraps Click underneath) that manages local libvirt+QEMU lab VMs from a single declarative YAML manifest (`Lvlab.yml`). It is meant for end-to-end integration testing of configuration-management code (Salt, Ansible, etc.) on a developer workstation — not for production VM management. The standalone one-off scripts (`createvm`, `destroyvm` in `src/tkc_lvlab/scripts/`) are also Typer-based; `createvm` keeps a minimal `import click` only for `click.Choice` on `--network-type` and `click.BadParameter` in `--ip4` parsing. Both scripts operate on **raw libvirt domain names** (the name you pass is the domain name — no `oneoff-` prefix; this reverses the earlier "Phase 6 architecture lock"). `createvm` resolves `--distro` against a built-in image catalog (`BUILTIN_IMAGES`, same dict schema as an `Lvlab.yml` `images:` entry) merged with the `images:` section of an `Lvlab.yml` in the current directory if present (manifest wins on a name collision); it derives `os_variant`/`username` from the image key unless the entry overrides them, and shares the `/var/lib/libvirt/images/lvlab/cloud-images` cache with `lvlab up`. Per-VM state lands under `/var/lib/libvirt/images/lvlab/oneoff/<vm_name>/`. `destroyvm` does not read `Lvlab.yml`, but because it acts on raw names it will remove a manifest VM if given that VM's actual `<vm_name>_<env>` domain name.
